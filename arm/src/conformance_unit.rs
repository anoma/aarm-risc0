...
impl ConformanceUnit {
    ...
    #[cfg(feature = "prove")]
    pub fn create(witness: &ConformanceWitness, proof_type: ProofType) -> Result<Self, ArmError> {
        let (proof, instance) = prove(CONFORMANCE_PK, witness, proof_type)?;
        Ok(ConformanceUnit { proof, instance })
    }

    /// Verifies the compliance proof against the instance using the constant verifying key.
    pub fn verify(&self) -> Result<(), ArmError> {
        verify_proof(&CONFORMANCE_VK, &self.instance, &self.proof)
    }
    ...
    pub fn get_instance(&self) -> Result<ConformanceInstance, ArmError> {
